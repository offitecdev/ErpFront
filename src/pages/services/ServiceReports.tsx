...
export const ServiceReports = () => {
    ...
    const load = async (nextSearch = search) => {
        setLoading(true);
        try {
            setReports(await projectApi.listAllReports({ search: nextSearch }));
        } catch (e: any) {
    ...
    useEffect(() => {
        if (view !== 'general' || projectsLoaded) return;
        void (async () => {
            try {
    ...
    useEffect(() => {
        if (view !== 'delivery' || deliveryLoaded) return;
        void (async () => {
            try {
                setDeliveryReports(await deliveryReportApi.list());
