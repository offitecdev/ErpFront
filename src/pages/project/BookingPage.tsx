...
export const BookingPage = () => {
    ...
    const load = async () => {
        ...
        try {
            ...
            toast.error(e.response?.data?.error ||t('auto.randevu_saatleri_yuklenemedi'));
        } finally {
            setLoading(false);
        }
    };
        ...
        <main className="min-h-screen bg-slate-50 px-4 py-8 font-sans text-slate-900">
            <section className="mx-auto max-w-3xl rounded-md border border-slate-200 bg-white">
                <div className="border-b border-slate-100 px-6 py-5">
                    ...
                    <h1 className="text-2xl font-semibold">{done ?t('auto.randevunuz_alindi') :t('auto.montaj_randevusu_secin')}</h1>
                    <p className="mt-2 text-sm text-slate-500">{projectName ||t('nav.projects')}{t('auto.icin_size_uygun_saatlerden_birini_secin')}</p>
                </div>

                <div className="p-6">
                    {done ? (
                        <div className="rounded-md border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">{t('auto.seciminiz_kaydedildi_bu_saat_artik_diger_musteri')}</div>
                    ) : (
                        <>
                            {loading && <div className="h-24 animate-pulse rounded bg-slate-100" />}
                            {!loading && slots.length === 0 && (
                                <div className="rounded-md border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">{t('auto.su_an_musait_saat_bulunamadi_lutfen_firma_ile_il')}</div>
                            )}
                ...
                {!done && (
                    <div className="flex justify-end border-t border-slate-100 bg-slate-50/70 px-6 py-4">
                        <Button disabled={!selected} loading={loading} onClick={book}>{t('auto.randevuyu_onayla')}</Button>
                    </div>
                )}
            </section>
        </main>
