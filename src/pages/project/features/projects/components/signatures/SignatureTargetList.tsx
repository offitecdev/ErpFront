...
const Group = ({ title, count, empty, children }: { title: string; count: number; empty?: string; children?: React.ReactNode }) => (
...
export const SignatureTargetList = ({
    ...
    return (
        <div className="space-y-4">
            ...
            <Group title={t('signatures.groups.field')} count={fieldReports.length} empty={fieldReports.length === 0 ? t('signatures.noneField') : undefined}>
            ...
            <Group title={t('signatures.groups.delivery')} count={deliveries.length} empty={deliveries.length === 0 ? t('signatures.noneDelivery') : undefined}>
                {deliveries.map((d) => (
                    <Row
                        key={d.id}
            ...
            <Group title={t('signatures.groups.general')} count={1}>
                <Row
